			...
			if(!is_admin()){
				...
				if(is_array($docs) && isset($docs['error'])){
					...
					if(count($docs->_content)>0){
						...
						$output .= '<h3>'.$ctitle.'</h3>';
						$output .='<ul class="issuu-list small-block-grid-2  medium-block-grid-3 large-block-grid-4">';
						$count = 0;
						foreach($docs->_content as $d){
							...
							if($isInTags || $wantItAll){
								...
								if($img !='false'){
									$output .= '<li '.$selected.'><a class="issuu-view" href="'.$doc_link.'" '.$link_target.'><img src="http://image.issuu.com/'.$dId.'/jpg/page_1_thumb_large.jpg" ><div class="block-gris">'.$d->document->title.'</div></a></li>';
								}else
								{
